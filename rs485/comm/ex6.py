...
GPIO.output(RS485_DE_RE_PIN, GPIO.LOW)  # Initially in listening mode
...
def send_request(slave_id):
    ...
    time.sleep(0.05)  # Ensure mode switch stability
    ser.write(b'sa\n')  # Send the command
    print(f"Request sent: '{command.strip()}'")
    time.sleep(0.05)  # Ensure the command is fully sent
    ...
    while True:
        ...
        if choice == '1':
            ...
            if slave_id in ['1', '2']:
                ...
                print("Invalid input. Please enter a valid slave number (1, 2).")
        elif choice == '2':
            print("Exiting.")
            ser.close()
            GPIO.cleanup()
