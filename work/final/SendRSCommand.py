...
try:
    ser = serial.Serial(
        port='/dev/serial0',            # Specify the correct serial port (e.g., /dev/serial0)
        baudrate=19200,                 # Set baud rate for serial communication
    ...
    # If there's an issue connecting to the serial port, print the error and exit
    print(f"Failed to connect to serial port: {e}")
    exit(1)

# Function to send a command via RS485
def send_request(command):
    full_command = f'{command}\n'            # Append newline to the command string
    GPIO.output(RS485_DE_RE_PIN, GPIO.HIGH)  # Switch GPIO pin to HIGH for transmit mode
    time.sleep(0.05)                         # Short delay before transmission
    ser.write(full_command.encode('utf-8'))  # Send the command as bytes over the serial connection
    print(f"Request sent: '{full_command.strip()}'")  # Print the sent command for confirmation
    time.sleep(0.05)                         # Short delay after transmission
    GPIO.output(RS485_DE_RE_PIN, GPIO.LOW)   # Switch GPIO pin back to LOW for receive mode
...
def receive_response():
    ...
    while True:
        if ser.in_waiting > 0:  # Check if there is data available to read from the serial buffer
            try:
                # Read a line of data from the serial port and decode it
                data = ser.readline().decode('utf-8', errors='ignore').strip()
                if data.isprintable():  
                    responses.append(data)  # Append valid data to the responses list
            except UnicodeDecodeError:
                # If there is an error in decoding, skip this data
                print("Received non-UTF-8 data, skipping...")
                continue
        if time.time() > timeout:  # Break the loop if the 2-second timeout has passed
            break
...
def main():
    ...
    while True:
        command = input("Enter command to send (or type 'exit' to quit): ")  # Prompt for user input

        if command.lower() == 'exit':   # Check if the user wants to exit the program
            print("Exiting.")           
        ...
        if responses:                   # If any responses were received
            ...
            print("No valid response received.")  # Notify the user if no response was received
...
if __name__ == "__main__":
    try:
        ...
        ser.close()                             # Close the serial connection
        GPIO.cleanup()                          # Clean up GPIO settings
        print("\nProgram terminated by user.")  # Notify the user the program has ended
